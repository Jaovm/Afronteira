...
@st.cache_data
def get_ifix_data(start_date, end_date):
    """Busca retornos mensais do IFIX via XFIX11 (ETF que replica o índice de FIIs)."""
    try:
        ifix = yf.download("XFIX11.SA", start=start_date, end=end_date, progress=False)
        if ifix.empty:
            return pd.Series(dtype="float64")
        prices = ifix["Adj Close"] if "Adj Close" in ifix.columns else ifix.iloc[:, 0]
        returns = prices.resample("ME").last().pct_change().dropna()
        returns.name = "IFIX"
        return returns
    except Exception as e:
        st.error(f"Erro no benchmark IFIX: {e}")
        return pd.Series(dtype="float64")
...
with st.spinner("📡 Carregando cotações, Ibovespa, IFIX e CDI..."):
    ...
    ifix_ret  = get_ifix_data(api_start, end_date)
...
ifix_ret       = ifix_ret.reindex(master_df.index).fillna(0)
...
ifix_accum     = (1 + ifix_ret.reindex(port_ret.index).fillna(0)).cumprod() * 100
ifix_aligned   = ifix_ret.reindex(port_ret.index).fillna(0)
...
with tab_perf:
    ...
    for name, y, color, width, dash in [
        ...
        ("IFIX",           ifix_accum.values, CORP["purple"],   2.0, "longdash"),
    ...
    for label_p, n in periods.items():
        if len(port_ret) >= n:
            ...
            f_ret = (1 + ifix_aligned.tail(n)).prod() - 1
...
with tab_corr:
    ...
    # Beta médio ponderado vs Ibovespa e vs IFIX
    ibov_for_beta = ibov_ret.reindex(corr_clean.index)
    ifix_for_beta = ifix_ret.reindex(corr_clean.index)

    def _weighted_beta(bench_series):
        """Beta ponderado do portfólio contra uma série de benchmark qualquer."""
        betas_local = {}
        for ticker in valid_cols:
            s = corr_clean[ticker].dropna()
            common = s.index.intersection(bench_series.dropna().index)
            if len(common) >= 12:
                x = bench_series.loc[common].values
                y = s.loc[common].values
                slope, _, _, _, _ = stats.linregress(x, y)
                betas_local[ticker] = slope
        if betas_local:
            b_port = sum(weights.get(t, 0) * b for t, b in betas_local.items())
            total_w = sum(weights.get(t, 0) for t in betas_local)
            return b_port / total_w if total_w > 0 else np.nan
        return np.nan

    beta_port      = _weighted_beta(ibov_for_beta)
    beta_port_ifix = _weighted_beta(ifix_for_beta)
    ...
    for col_m, lbl, val, ac in [
        ...
        (m5, "Beta do Portfólio (vs IFIX)",  f"{beta_port_ifix:.2f}" if not np.isnan(beta_port_ifix) else "N/A", CORP["danger"]),
    ]:
    ...
    st.caption(
        ...
        "**Beta** = sensibilidade estimada a cada benchmark (Ibovespa/IFIX)."
    )
    ...
    # ── Mapa de calor de correlação com IFIX ──────────────────────────────────
    st.markdown("<div class='section-header'>🏢 Correlação de Cada Ação com o IFIX</div>",
                unsafe_allow_html=True)
    ...
    for ticker in valid_cols:
        s      = corr_clean[ticker].dropna()
        common = s.index.intersection(ifix_for_beta.dropna().index)
        if len(common) >= 12:
            corr_v = s.loc[common].corr(ifix_for_beta.loc[common])
            ifix_corrs[ticker] = corr_v
            sl, ic, _, _, _ = stats.linregress(ifix_for_beta.loc[common].values, s.loc[common].values)
            ifix_betas_all[ticker] = sl

    if ifix_corrs:
        df_ifix_corr = pd.DataFrame({
            "Ticker":          list(ifix_corrs.keys()),
            "Corr. c/ IFIX":  [ifix_corrs[t] for t in ifix_corrs],
            "Beta vs IFIX":    [ifix_betas_all.get(t, np.nan) for t in ifix_corrs],
        }).sort_values("Corr. c/ IFIX", ascending=True)

        fig_bar_ifix = go.Figure()
        fig_bar_ifix.add_trace(go.Bar(
            x=df_ifix_corr["Ticker"],
            y=df_ifix_corr["Corr. c/ IFIX"],
            marker_color=[CORP["success"] if v < 0.3 else CORP["warning"] if v < 0.6 else CORP["danger"]
                          for v in df_ifix_corr["Corr. c/ IFIX"]],
            text=[f"{v:.2f}" for v in df_ifix_corr["Corr. c/ IFIX"]],
            textposition="outside",
            hovertemplate="<b>%{x}</b><br>Corr: %{y:.3f}<extra></extra>",
        ))
        fig_bar_ifix.add_hline(y=0.5, line_dash="dash", line_color=CORP["warning"],
                                annotation_text="Corr. Moderada (0.5)", line_width=1.5)
        fig_bar_ifix.update_layout(**corp_layout(
            title=dict(text="Correlação de Cada Ação com o IFIX (XFIX11)", font_size=13),
            xaxis=dict(title="Ação", showgrid=False),
            yaxis=dict(title="Correlação", range=[-1.1, 1.15], gridcolor="#F1F5F9"),
            showlegend=False,
            height=350, margin=dict(t=55, b=50, l=50, r=20),
        ))
        st.plotly_chart(fig_bar_ifix, use_container_width=True)

        st.dataframe(
            df_ifix_corr.sort_values("Corr. c/ IFIX", ascending=False)
              .style.format({"Corr. c/ IFIX": "{:.3f}", "Beta vs IFIX": "{:.3f}"})
              .background_gradient(cmap="RdYlGn_r", subset=["Corr. c/ IFIX"], vmin=-0.5, vmax=1)
              .set_properties(**{"text-align": "center"})
              .set_table_styles([{"selector": "th", "props": [("text-align", "center")]}]),
            use_container_width=True, hide_index=True,
        )
        st.caption("💡 Correlação com o IFIX tende a ser mais baixa que com o Ibovespa, já que fundos "
                   "imobiliários têm dinâmica de preço distinta das ações — valores baixos ou negativos "
                   "aqui indicam potencial de diversificação, não um problema.")
